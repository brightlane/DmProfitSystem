...
class ManyChatGlobalEmpire {
  constructor() {
    ...
    this.siteUrl    = 'https://bot.yourdomain.com'; // CHANGE THIS TO YOUR DOMAIN

    // Your 12 ManyChat affiliate links
    this.affiliates = [
  ...
  async run() {
    console.log('🤖 MANYCHAT 240K GLOBAL EMPIRE — STARTUP');
  ...
  generatePage(country, currency, template, planName, affLink, price) {
    ...
    const desc  = `Earn 50% commission via ManyChat affiliate program in ${currency.code} (${country.toUpperCase()}) pricing.`;
...
<head>
  ...
  <link rel="alternate" hreflang="en-us" href="${this.siteUrl}/manychat/us-${template}-usd.html" />
  <link rel="alternate" hreflang="en-gb" href="${this.siteUrl}/manychat/gb-${template}-gbp.html" />
...
<body>
  <!-- Mobile promo (1 clear CTA) -->
  <div class="mobile-promo">
    <h2>🤖 ManyChat ${planName}</h2>
    <p><strong>${currency.symbol}${price}/mo</strong> via ManyChat affiliate</p>
  ...
  <div class="hero">
    ...
    <p>${currency.symbol}${price}/mo ManyChat affiliate plan in ${currency.code}</p>
    <a href="${affLink}" class="price">🚀 Get 50% for 12 Months</a>
  </div>

  <!-- Features / feeders -->
  <ul class="features">
    <li>✅ 50% commission on ManyChat paid plans for 12 months</li>
    <li>✅ Free 14‑day ManyChat trial via affiliate link</li>
    <li>✅ Works with TikTok, IG, WhatsApp, SMS, Messenger</li>
    <li>✅ Global pricing in ${currency.code} (${country.toUpperCase()})</li>
  </ul>
